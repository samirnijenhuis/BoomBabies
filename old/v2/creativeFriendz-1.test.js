// test/CreativeFriendz-1.test.js
// Load dependencies
...
// Start test block
describe('CreativeFriendzClub1', function () {
  before(async function () {
    this.Token = await ethers.getContractFactory('CreativeFriendzClub1');
  ...
  //     const signers = await ethers.getSigners();
  //
  //   this.token = await this.Token.deploy("ipfs://some-test-uri", /*team:*/ [],  /*whitelist:*/ signers.map((s) => s.address));
  //   await this.token.deployed();
  //
  ...
  it("can only be minted for the mint price", async function() {
    ...
    await expect(
        this.token.connect(signers[2]).mint(1, {value: parseEther('0.4999999').toString()})
    ).to.be.revertedWith("Amount of ether sent does not match total mint amount");

    this.token.connect(signers[2]).mint(1, {value: parseEther('0.5').toString()})
  })
  ...
  it("only allows 1 token per mint", async function() {
    ...
    await expect(
        this.token.connect(signers[2]).mint(2, {value: parseEther('0.5').toString()})
  ...
  it("only allows whitelisted people to mint", async function() {
    ...
    ).to.be.revertedWith("Whitelist entry required");
